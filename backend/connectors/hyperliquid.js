const DEFAULT_ENDPOINT = process.env.HYPERLIQUID_ROYALTY_ENDPOINT;
...
async function fetchRoyalties(endpoint) {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Hyperliquid royalties: ${response.status} ${response.statusText}`);
  }
...
async function getHyperliquidRoyalties(endpoint) {
  try {
    return await fetchRoyalties(endpoint);
  } catch (error) {
    console.error("Hyperliquid royalty fetch error:", error);
    return { totalRoyalties: 0n };
  }
}
